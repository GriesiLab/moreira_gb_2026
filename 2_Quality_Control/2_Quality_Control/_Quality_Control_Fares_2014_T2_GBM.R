###############################################################################
# Script:        Quality Control - Fares_2017 (GSE77128)
# Author:        Gustavo Bueno Moreira
# Last update:   2026-02-16
#
# Purpose:
#   - Perform an exploratory analysis of RNA-seq data from the selected study,
...
#   - DATA/GSE77128_raw_counts_GRCh38.p13_NCBI.tsv.gz
...
setwd("~/Insync/cgustavobm2018@gmail.com/Google Drive - Shared with me/_Projeto_Pi_Fapesp_2022/2a_b_WGCNA_public_data_fresh_cultivated_HSC/Type_2_Studies/Fares_2016")
...
convert_to_ensembl <- function(expDataRaw) {
  ...
  if (id_type != "Ensembl") {
    ...
    if (is.na(expr_col)) {
      stop("No numeric column found for expression sorting.")
    }
...
names(metadata)[names(metadata) == "sorted cell population:ch1"] <- "LT_enrichment_fct"
names(metadata)[names(metadata) == "data_processing.3"] <- "Genome_build"

metadata <- metadata %>%
  mutate(
    title = gsub("\\+", "High", title),
    title = gsub("\\-", "Neg", title),
    title = gsub("^(Exp100_d7_|Exp102_d7_)", "", title),
    
    Genome_build = case_when(
      Genome_build == "Genome_build: hg19" ~ "GRCH38",
      TRUE ~ as.character(Genome_build)
    ),
    
    LT_enrichment = case_when(
      LT_enrichment_fct == "EPCR+" ~ 3,
      LT_enrichment_fct == "EPCRlow" ~ 2,
      LT_enrichment_fct == "EPCR-" ~ 1,
      TRUE ~ NA_real_
    ),
    
    Sample_fct = rep(
      c("Sample 1", "Sample 2", "Sample 1", "Sample 2", "Sample 3",
    ...
    Imunophenotype = case_when(
      LT_enrichment_fct == "EPCR+" ~ "CD34+/CD45RA-/EPCR+",
      LT_enrichment_fct == "EPCR-" ~ "CD34+/CD45RA-/EPCR-",
      LT_enrichment_fct == "EPCRlow" ~ "CD34+/CD45RA-/EPCRlow",
      TRUE ~ NA_character_
    ),
    ...
    Individual = case_when(
      ...
      Sample_fct == "Sample 4" ~ 4,
      Sample_fct == "Sample 5" ~ 5,
      TRUE ~ NA_real_
    ),
    
    Experiment = case_when(
      grepl("^Exp100", source_name_ch1) ~ 1,
      grepl("^Exp102", source_name_ch1) ~ 2,
      TRUE ~ NA_real_
    )
  )

exp_file <- "DATA/GSE77128_raw_counts_GRCh38.p13_NCBI.tsv.gz"
...
# Metrics: Batch Correction p-value Threshold: 0.8;  k Value: 1
...
write.csv(expData, file = "DATA/counts_RUVseq_Normalized.csv")

message("3) Optional: removing low-variance genes if needed...")
...
if (nrow(expData) > 15000) {
  ...
  expData2b <- expData
  message(paste("The number of genes is", nrow(expData), "(<= 15,000). Low variance gene removal was skipped."))
}
