...
#
# Purpose:
#   - Perform an exploratory analysis of RNA-seq data from a selected study,
...
#   4) Low-variance genes table exported to CSV (if filtering is applied)
#
# Inputs:
#   - `expData`: raw count matrix (genes × samples) loaded from text file
#   - `metadata`: sample annotations loaded from GEO
#   - DATA/GSE125345_LTHSC_ST_HSC_CMP_GMP_MLP.tsv
...
filter_genes_by_tpm <- function(expData, metadata) {
  ...
  if (is.na(n) || n <= 0) {
    stop("Invalid value for n. It must be a positive integer.")
  }
...
normalize_data <- function(res, expData, metadata, HKgenes) {
  ...
  expData_normalized <- as.data.frame(RUV$normalizedCounts)
...
get_pca_top_genes <- function(results_pca, pc = 1, top = 100, mart) {
  message("Extracting PCA contributions...")
  ...
  if (length(pca_contrib$Ensembl) == 0) {
    stop("Error: No Ensembl identifiers found in PCA results.")
  }
  ...
  converted_genes <- getBM(
    attributes = c("ensembl_gene_id", "hgnc_symbol"),
    filters = "ensembl_gene_id",
    values = pca_contrib$Ensembl,
    mart = mart
  )
  
  if (nrow(converted_genes) == 0) {
    stop("Error: No genes were converted.")
  }
...
metadata <- metadata %>%
  mutate(
    Genome_build = case_when(
      Genome_build == "Genome_build: GRCH38" ~ "GRCH38",
      TRUE ~ as.character(Genome_build)
    ),
    LT_fct = case_when(
      `cell type:ch1` == "Long-term hematopoietic stem cells (LT-HSC)" ~ "LT",
      `cell type:ch1` == "Short-term hematopoietic stem cells (ST-HSC)" ~ "ST",
      `cell type:ch1` == "Common myeloid progenitors (CMP)" ~ "CMP",
      `cell type:ch1` == "Granulocyte monocyte Progenitors (GMP)" ~ "GMP",
      `cell type:ch1` == "Multilymphoid Progenitors (MLP)" ~ "MLP",
      TRUE ~ NA_character_
    ),
    LT = case_when(
      LT_fct == "LT" ~ 3,
      LT_fct == "ST" ~ 2,
      LT_fct %in% c("CMP", "GMP", "MLP") ~ 1,
      TRUE ~ NA_real_
    ),
    Sample_fct = case_when(
      `cord blood pool:ch1` == "7" ~ "Sample 1",
      `cord blood pool:ch1` == "8" ~ "Sample 2",
      `cord blood pool:ch1` == "9" ~ "Sample 3",
      TRUE ~ NA_character_
    ),
    Imunophenotype = case_when(
      LT_fct == "LT" ~ "CD34+/CD38-/CD45RA-/CD90+/CD49f+",
      LT_fct == "ST" ~ "CD34+/CD38-/CD45RA-/CD90-/CD49f+",
      LT_fct == "GMP" ~ "CD34+/CD38+/CD10-/CD7-/CD45RA+",
      TRUE ~ NA_character_
    ),
    Individual = case_when(
      Sample_fct == "Sample 1" ~ 1,
      Sample_fct == "Sample 2" ~ 2,
      Sample_fct == "Sample 3" ~ 3,
      TRUE ~ NA_real_
    )
  ) %>%
  rename(
    LT_enrichment = LT,
    LT_enrichment_fct = LT_fct
  )

exp_file <- "DATA/GSE125345_LTHSC_ST_HSC_CMP_GMP_MLP.tsv"
separator <- detect_separator(exp_file)

expDataRaw <- read.delim(exp_file, sep = separator)
expData <- expDataRaw

rownames(expData) <- expData$ENSEMBL_ID
...
repeat {
  ...
  if (outlier_resp == "yes") {
    ...
    repeat {
      ...
      if (batch_resp == "yes") {
        message("Proceed to RUVseq for batch effect removal.")
        break
...
# Metrics: Batch Correction p-value Threshold: 0.3;  k Value: 5
...
clinical_subset <- as.data.frame(clinical[, c(47, 48)])
...
message("4) Initializing Ensembl mart for identifier conversion...")
