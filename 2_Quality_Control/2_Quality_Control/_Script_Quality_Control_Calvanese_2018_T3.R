...
#   - DATA/GSE111483_raw_counts_GRCh38.p13_NCBI.tsv
...
setwd("~/Insync/cgustavobm2018@gmail.com/Google Drive - Shared with me/_Projeto_Pi_Fapesp_2022/4a_Panel_validation_inSilico/Studies_QC/RNAseq/Calvanese_2019/")
...
names(metadata)[names(metadata) == "genotype/variation:ch1"] <- "LT_enrichment_fct"
...
metadata <- metadata %>%
  mutate(
    ...
    LT_enrichment = case_when(
      ...
      LT_enrichment_fct == "MLLT3 overexpression" ~ 1,
      TRUE ~ NA_real_
    ),
    Sample = rep(paste0("Sample ", 1:6), length.out = n()),
    Imunophenotype = case_when(
      LT_enrichment_fct %in% c("control", "MLLT3 overexpression") ~ "CD34+/CD38-/CD90+",
      TRUE ~ NA_character_
    ),
    Enrichment = case_when(
      LT_enrichment_fct == "control" ~ "Lower Enrichment",
      LT_enrichment_fct == "MLLT3 overexpression" ~ "Upper Enrichment",
      TRUE ~ NA_character_
    ),
    Cell_Source = "Fetal Liver"
  )

exp_file <- "DATA/GSE111483_raw_counts_GRCh38.p13_NCBI.tsv"
separator <- detect_separator(exp_file)

expDataRaw <- read.delim(exp_file, sep = separator, stringsAsFactors = FALSE)
...
# Convert first column IDs to Ensembl, then build count matrix
expDataRaw2 <- expDataRaw
expDataRaw2 <- convert_identifiers(expDataRaw2, to_id = "Ensembl", mart = mart)

# Expect: first column is GeneID; after conversion, we have Ensembl column.
# Keep only numeric columns as counts:
...
# Optional: remove duplicated rownames just in case
...
# Metrics: Batch Correction p-value Threshold: 0.8 ;  k Value: 5 
