###############################################################################
# Script:        QC Barreyro_2012
# Author:        Gustavo Bueno Moreira
# Last update:   2026-02-19
#
# Purpose:
#   - Perform quality control analysis on Affymetrix Human Gene 1.0 ST Array data,
...
#
# Inputs:
#   - Raw CEL files: DATA/GSE35010/*.CEL (Affymetrix Human Gene 1.0 ST Array)
#   - GEO metadata downloaded via GEOquery (GSE35010)
#   - BrainArray custom CDF packages (installed from local tar.gz):
#       - DATA/hugene10sthsensgprobe_25.0.0.tar.gz
#       - DATA/pd.hugene10st.hs.ensg_25.0.0.tar.gz
#
# Outputs:
#   - DATA/metadata.csv
...
setwd("~/Insync/cgustavobm2018@gmail.com/Google Drive - Shared with me/_Projeto_Pi_Fapesp_2022/4a_Panel_validation_inSilico/Studies_QC/Microarray/Barreyro_2012")

# 0.2) Options & constants -----------------------------------------------------
options(stringsAsFactors = FALSE)

# 0.3) Install platform-specific annotation packages (BrainArray) --------------
# Note: These are custom packages for the Affymetrix Human Gene 1.0 ST Array
# Note: Acess http://brainarray.mbni.med.umich.edu/Brainarray/Database/CustomCDF/25.0.0/ensg.asp
install.packages("DATA/hugene10sthsensgprobe_25.0.0.tar.gz", repos = NULL, type = "source")
install.packages("DATA/pd.hugene10st.hs.ensg_25.0.0.tar.gz", repos = NULL, type = "source")
...
library(oligo)
library(stringr)
library(pd.hugene10st.hs.ensg)
library(hugene10sthsensgprobe)
...
metadata <- metadata %>%
  filter(`karyotype:ch1` == "Healthy control") %>%
  mutate(
    LT_enrichment_fct = case_when(
      grepl("short term", title, ignore.case = TRUE) ~ "ST",
      grepl("long-term", title, ignore.case = TRUE) ~ "LT",
      grepl("Granulocytic monocytic", title, ignore.case = TRUE) ~ "GMP",
      TRUE ~ NA_character_
    ),
    LT_enrichment = case_when(
      LT_enrichment_fct %in% c("LT")  ~ 2,
      LT_enrichment_fct %in% c("ST")  ~ 1,
      LT_enrichment_fct %in% c("GMP") ~ 0,
      TRUE ~ NA_real_
    ),
    Imunophenotype = case_when(
      LT_enrichment_fct %in% c("LT")  ~ "Lin-/CD34+/CD38-/CD90+",
      LT_enrichment_fct %in% c("ST")  ~ "Lin-/CD34+/CD38-/CD90-",
      LT_enrichment_fct %in% c("GMP") ~ "Lin-/CD34+/CD38+/CD123+/CD45R+",
      TRUE ~ NA_character_
    ),
    Enrichment = case_when(
      LT_enrichment_fct %in% c("LT")  ~ "Upper Enrichment",
      LT_enrichment_fct %in% c("ST")  ~ "Moderate Enrichment",
      LT_enrichment_fct %in% c("GMP") ~ "Lower Enrichment",
      TRUE ~ NA_character_
    ),
...
data_folder <- "DATA/GSE35010"

cel_files <- list.celfiles(path = data_folder, full.names = TRUE)
length(cel_files)

raw_data <- read.celfiles(cel_files, pkgname = "pd.hugene10st.hs.ensg")
raw_data
...
rownames(expData) <- gsub("_at$", "", rownames(expData))
