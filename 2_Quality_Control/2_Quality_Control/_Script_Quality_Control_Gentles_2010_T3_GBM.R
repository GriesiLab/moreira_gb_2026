###############################################################################
# Script:        QC Gentles_2010
# Author:        Gustavo Bueno Moreira
# Last update:   2026-02-19
#
# Purpose:
#   - Perform quality control analysis on Affymetrix Human Genome U133 Plus 2.0 Array data,
#     including RMA normalization of CEL files (BrainArray CDF) and exploratory QC plots.
#
# Deliverables:
...
#   4) (Optional) Bone marrow-only subset exported to CSV
#
# Inputs:
#   - Raw CEL files: DATA/GSE24006/*.CEL (Affymetrix Human Genome U133 Plus 2.0 Array)
#   - GEO metadata downloaded via GEOquery (GSE24006)
#   - BrainArray custom CDF packages / files (installed or loaded locally):
#       - DATA/hgu133plus2hsensgprobe_25.0.0.tar.gz
#       - DATA/pd.hgu133plus2.hs.ensg_25.0.0.tar.gz
#       - DATA/HGU133Plus2_Hs_ENSG.cdf
#
# Outputs:
#   - DATA/metadata.csv
#   - DATA/normalized_expression_RMA.csv
#   - DATA/metadata_BM.csv
#   - DATA/normalized_expression_RMA_BM.csv
...
setwd("~/Insync/cgustavobm2018@gmail.com/Google Drive - Shared with me/_Projeto_Pi_Fapesp_2022/4a_Panel_validation_inSilico/Studies_QC/Microarray/Gentles_2010/")

# 0.2) Options & constants -----------------------------------------------------
options(stringsAsFactors = FALSE)

# 0.3) Install platform-specific annotation packages (BrainArray) --------------
# Note: These are custom packages for the Affymetrix Human Genome U133 Plus 2.0 Array
# Note: Acess http://brainarray.mbni.med.umich.edu/Brainarray/Database/CustomCDF/25.0.0/ensg.asp
install.packages("DATA/hgu133plus2hsensgprobe_25.0.0.tar.gz", repos = NULL, type = "source")
install.packages("DATA/pd.hgu133plus2.hs.ensg_25.0.0.tar.gz", repos = NULL, type = "source")
...
library(data.table)
library(affy)          # Affymetrix arrays (U133, etc.)
library(makecdfenv)    # Load custom CDF environments
library(stringr)
...
# NOTE:
# - I kept the same helper functions you provided.
# - Your script references check_id_type(), but it is not defined here.
#   If you use convert_identifiers() or get_pca_top_genes(), make sure check_id_type() exists.
...
metadata <- metadata %>%
  filter(str_to_lower(trimws(`disease state:ch1`)) == "normal") %>%
  mutate(
    LT_enrichment_fct = word(title, 2, sep = fixed("_"))
  )

colnames(metadata)[colnames(metadata) == "cell type (purified cell subset):ch1"] <- "Imunophenotype"
metadata$Imunophenotype <- gsub("([+-])", "\\1/", metadata$Imunophenotype)
metadata$Imunophenotype <- gsub("/$", "", metadata$Imunophenotype)
metadata$Imunophenotype <- gsub("/", "/", metadata$Imunophenotype)

cell_source_blocks <- c(8, 3, 8, 4, 4, 3, 1)
cell_source_labels <- c(
  "Bone Marrow", "Cord Blood", "Bone Marrow",
  "Cord Blood", "Bone Marrow", "Cord Blood",
  "Bone Marrow"
)
cell_source_vector <- rep(cell_source_labels, times = cell_source_blocks)
stopifnot(nrow(metadata) >= length(cell_source_vector))
...
metadata <- metadata %>%
  mutate(
    LT_enrichment = case_when(
      ...
      LT_enrichment_fct %in% c("RA-pos") ~ 0,
      TRUE ~ NA_real_
    ),
    Enrichment = case_when(
      LT_enrichment_fct %in% c("HSC") ~ "Upper Enrichment",
      LT_enrichment_fct %in% c("MPP") ~ "Moderate Enrichment",
      LT_enrichment_fct %in% c("CMP", "GMP", "MEP") ~ "Low Enrichment",
      TRUE ~ NA_character_
    ),
    Cell_Source = dplyr::case_when(
      dplyr::row_number() <= length(cell_source_vector) ~ cell_source_vector[dplyr::row_number()],
      TRUE ~ NA_character_
    ),
    Cell_Source = factor(Cell_Source, levels = c("Bone Marrow", "Cord Blood"))
  )

metadata <- metadata[-c(28:31), ]

##### 1.2 Read Expression Data (CEL -> RMA) ###################################

data_folder <- "DATA/GSE24006"
...
colnames(expData) <- sub("\\.CEL$", "", colnames(expData), ignore.case = TRUE)
...
# Optional: save Bone Marrow-only subset --------------------------------------
metadataBM <- metadata %>% filter(Cell_Source == "Bone Marrow")
expDataBM <- expData[, colnames(expData) %in% rownames(metadataBM)]

write.csv(metadataBM, file = "DATA/metadata_BM.csv")
write.csv(expDataBM, file = "DATA/normalized_expression_RMA_BM.csv")
