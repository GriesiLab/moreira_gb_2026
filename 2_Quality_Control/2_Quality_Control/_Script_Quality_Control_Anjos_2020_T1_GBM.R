...
perform_pca <- function(expData, metadata, population_column = NULL, sample_column = NULL) {
  ...
  if (!is.na(population_column)) {
    pop_order <- c("LT-HSC", "CD90-CD49f+", "CD90+CD49f-", "ST-HSC")
...
metadata <- metadata %>%
  mutate(
    ...
    LT_enrichment_fct = case_when(
      source_name_ch1 == "CD90+CD49f+" ~ "LT-HSC",
      source_name_ch1 == "CD90+CD49f-" ~ "CD90+CD49f-",
      source_name_ch1 == "CD90-CD49f+" ~ "CD90-CD49f+",
      source_name_ch1 == "CD90-CD49f-" ~ "ST-HSC",
      TRUE ~ NA_character_
    ),
    LT_enrichment = case_when(
      source_name_ch1 == "CD90+CD49f+" ~ 3,
      source_name_ch1 == "CD90+CD49f-" ~ 2,
      source_name_ch1 == "CD90-CD49f+" ~ 2,
      source_name_ch1 == "CD90-CD49f-" ~ 1,
      TRUE ~ NA_real_
    ),
    Sample_fct = case_when(
      grepl("S1$", title) ~ "Sample 1",
      grepl("S2$", title) ~ "Sample 2",
      grepl("S3$", title) ~ "Sample 3",
      TRUE ~ NA_character_
    ),
    Imunophenotype = case_when(
      source_name_ch1 == "CD90+CD49f+" ~ "CD34+/CD38-/CD45RA-/CD90+/CD49f+",
      source_name_ch1 == "CD90-CD49f+" ~ "CD34+/CD38-/CD45RA-/CD90-/CD49f+",
      source_name_ch1 == "CD90+CD49f-" ~ "CD34+/CD38-/CD45RA-/CD90+/CD49f-",
      source_name_ch1 == "CD90-CD49f-" ~ "CD34+/CD38-/CD45RA-/CD90-/CD49f-",
      TRUE ~ NA_character_
...
exp_file <- "DATA/GSE154588_combined_gene.exp.txt"
separator <- detect_separator(exp_file)

expDataRaw <- read.delim(exp_file, sep = separator, row.names = 1)
expData <- expDataRaw
...
# Create a default mart for identifier conversion (used later by get_pca_top_genes)
...
# Metrics: Batch Correction p-value Threshold: 0.3;  k Value: 3
