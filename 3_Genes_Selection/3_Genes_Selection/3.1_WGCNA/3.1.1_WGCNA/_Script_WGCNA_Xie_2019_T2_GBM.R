...
#
# Purpose:
#   - Run WGCNA on variance-stabilized RNA-seq counts for Xie (2019) Type 2
#     dataset, generating network modules, kME tables, module eigengenes, and
#     module-trait correlation summaries and plots (including an LDA-coded trait).
#
# Deliverables:
...
#### 0.3) Packages ####
library(WGCNA)    # coexpression network analysis (modules, eigengenes, kME)
library(multtest) # multiple testing utilities used in kME adjustment workflow
library(biomaRt)  # Ensembl BioMart access for gene annotation
library(coin)     # permutation-based tests (independence_test, pvalue)
library(DESeq2)   # variance stabilizing transformation (VST) for RNA-seq counts

library(dplyr)    # data manipulation (joins, mutate, filter, distinct)
library(tidyr)    # reshaping (pivot_wider)
library(tibble)   # tibble construction (tibble::tibble)

library(ggplot2)  # plotting (soft-threshold, volcano, exports)
...
moduleTrait_long$Study <- "Xie_Type2"
