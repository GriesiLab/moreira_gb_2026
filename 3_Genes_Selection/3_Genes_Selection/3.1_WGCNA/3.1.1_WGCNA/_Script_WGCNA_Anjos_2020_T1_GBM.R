###############################################################################
# Script:        WGCNA Anjos Afonso
# Author:        Gustavo Bueno Moreira
# Last update:   2026-02-16
#
# Purpose:
#   - Run WGCNA on variance-stabilized RNA-seq counts (fresh/uncultured cells)
#     from Anjos-Afonso et al., generating network modules, kME tables, module
...
#   - RESULTS/WGCNA/2_Cluster_dendogram_overview.pdf
#   - RESULTS/WGCNA/3_kmeTable.csv
#   - RESULTS/WGCNA/4_ME.csv
#   - RESULTS/WGCNA/New/4_moduleBarplots.pdf
...
setwd("~/Insync/cgustavobm2018@gmail.com/Google Drive - Shared with me/_Projeto_Pi_Fapesp_2022/2a_b_WGCNA_public_data_fresh_cultivated_HSC/Type_1_Studies/Anjos-Afonso_etal_2021")

#### 0.2) Options & constants ####
options(stringsAsFactors = FALSE)

#### 0.3) Packages ####
####WGCNA - Xie - Uncultured cells####
#Description: In this script we ran WGCNA on fresh cell data from Xie , the study had 4 cell populations and the data provided were in counts 
...
expData  = read.csv("DATA/counts_RUVseq_Normalized.csv", row.names = 1)
...
png("RESULTS/WGCNA/2_Cluster_dendrogram.png", width = 7, height = 7, units = "in", res = 300)
...
png("RESULTS/WGCNA/2_Cluster_dendogram_overview.png", width = 14.67, height = 14.67, units = "in", res = 150)
par(mar = c(5, 5, 4, 2))
plot(
  METree,
...
pdf("RESULTS/WGCNA/2_Cluster_dendogram_overview.pdf")
...
message("2.1) Exporting kME table (with BH-adjusted p-values) and BioMart annotation...")
...
legend_labels <- unique(metadata$LT_imunophenotype)
legend_colors <- colors[!duplicated(metadata$LT_imunophenotype)]

pdf("RESULTS/WGCNA/New/4_moduleBarplots.pdf", height = 5, width = 15)
mod = colnames(me)
for (m in mod) {
...
plot <- ggplot(df_lt, aes(x = Correlation, y = NegLog10FDR_plot, fill = Module, color = Module)) +
  ...
  labs(
    title = "Correlation of Eigengenes with LT-HSC samples - Anjos",
