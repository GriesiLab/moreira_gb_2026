...
#     from Xie et al., generating network modules, kME tables, module
...
legend_labels <- unique(metadata$LT_fct)
legend_colors <- colors[!duplicated(metadata$LT_fct)]
...
moduleTrait_long$Study <- "Xie_Type1"
...
plot <- ggplot(df_lt, aes(x = Correlation, y = NegLog10FDR_plot, fill = Module, color = Module)) +
  ...
  labs(
    title = "Correlation of Eigengenes with LT-HSC samples - Xie",
