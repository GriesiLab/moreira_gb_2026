###############################################################################
# Script:        Functional Enrichment GO BP WGCNA Modules Xie
# Author:        Gustavo Bueno Moreira
# Last update:   2026-02-16
#
# Purpose:
#   - Perform GO enrichment (clusterProfiler::enrichGO + simplify) for selected
...
#
# Inputs:
#   - DATA/counts_Final.csv        (gene universe; first column = Ensembl)
...
library(enrichplot)       # enrichment plotting utilities
library(dplyr)            # data manipulation
library(stringr)          # string handling (text wrapping)
...
run_enrichment_analysis <- function(
  ...
  for (mod in selected_modules) {
    ...
    message(
      paste0(
        "[INFO] Running enrichment for module '", mod,
        "' with ", length(module_gene_list), " genes..."
      )
    )
    ...
    csv_path <- file.path(
      output_dir,
      paste0(output_prefix, "_", mod, "_GO_", ontology, ".csv")
    )
    rds_path <- file.path(
      output_dir,
      paste0(output_prefix, "_", mod, "_GO_", ontology, ".rds")
    )
...
universe_genes <- read.csv("DATA/counts_Final.csv")
...
run_enrichment_analysis(
  ...
  selected_modules   = c(
    "greenyellow", "green", "grey60",
    "darkturquoise", "lightgreen", "magenta", "brown"
  ),
  ontology           = "BP",
  p_value_cutoff     = 0.05,
  min_gene_set_size  = 10,
...
for (file in results_files) {
  ...
  p <- ggplot(
    ...
    aes(
      x = GeneRatio,
      y = reorder(Description_wrapped, GeneRatio)
    )
  ) +
